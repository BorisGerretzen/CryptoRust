...
fn main() {
    ...
    let (client, mediator) = crypto::keygen(&attributes, &public, &master, rng).unwrap();

    let access_tree = AccessTreeParser::new("a & b").parse().unwrap();
    let message = "Hello".as_bytes().to_vec();
    let encrypted = crypto::encrypt(rng.gen(), &message, &public, &access_tree, rng).unwrap();
    let mediated = crypto::m_decrypt(&encrypted, &mediator).unwrap();
    let decrypted = crypto::decrypt(&encrypted, &client, &mediated).unwrap();
